...
function agencyAliases(agency = '') {
  const raw = String(agency || '').replace(/\s+/g, '').trim();
  if (!raw) return [];
  const stripped = raw
    .replace(/^서울특별시교육청/, '')
    .replace(/^서울시교육청/, '')
  ...
  if (raw === '본청' || raw === '서울특별시교육청' || raw === '서울시교육청') aliases.push('서울시교육청본청');
  if (raw === '서울시교육청본청') aliases.push('본청', '서울특별시교육청');
  return [...new Set(aliases.filter(Boolean))];
}

function agencySearchTerms(agency = '') {
  const aliases = agencyAliases(agency);
  const terms = new Set(aliases);
  for (const alias of aliases) {
    terms.add(alias.replace(/교육지원청$/, ''));
    terms.add(alias.replace(/도서관$/, ''));
    terms.add(alias.replace(/평생학습관$/, ''));
  }
...
function pageUrls(listUrl, pageLimit = 1, { agency = '', year = '', month = '' } = {}) {
  ...
  const aliases = agencyAliases(agency);
  const searchTerms = agencySearchTerms(agency);

  for (const alias of aliases) {
    if (AGENCY_CATEGORY_HINTS[alias]) {
      bases.unshift(listUrl.replace('/ctgynone/', `/${AGENCY_CATEGORY_HINTS[alias]}/`));
    }
  }
  ...
  if (agency) {
    for (const term of searchTerms) {
      for (const key of ['searchWrd', 'searchKeyword', 'searchWord', 'q']) {
        bases.push(setQuery(listUrl, { [key]: term }));
        bases.push(setQuery(listUrl, { [key]: `${year}년 ${month}월 ${term} 업무추진비` }));
      }
      bases.push(setQuery(listUrl, { searchCnd: '0', searchWrd: term }));
      bases.push(setQuery(listUrl, { searchCnd: '1', searchWrd: term }));
    }
  }
...
function extractRowsFromHtml(html, pageUrl, { year, month, agency = '', latest = false }) {
  ...
  const agencyTerms = agencyAliases(agency);
  ...
  for (const chunk of chunks) {
    ...
    if (agencyTerms.length && !agencyTerms.some(term => compact.includes(term))) continue;
...
async function findPost({ source = 'chief', year, month, agency = '', latest = false }) {
  ...
  const limit = latest ? 3 : (source === 'org' ? 25 : 2);
  const candidates = [];
  const notices = [];
